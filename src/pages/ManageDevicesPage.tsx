...
import {
    Map, Cpu, Plus, Edit, Trash2, Search, Filter, RefreshCw, X, Save, Thermometer, Droplets, Sun, Wind
} from 'lucide-react';
...
export const ManageDevicesPage: React.FC = () => {
    ...
    const handleSaveArea = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert(`Lỗi: ${error.response?.data?.error || 'Không thể lưu khu vực'}`);
        }
    };
    ...
    const handleDeleteArea = async (id: number, name: string) => {
        ...
        try {
            ...
            alert(`Lỗi: ${error.response?.data?.error || 'Khu vực này đang chứa thiết bị, không thể xóa'}`);
        }
    };
    ...
    const handleSaveDevice = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert(`Lỗi: ${error.response?.data?.error || 'Không thể lưu thiết bị'}`);
        }
    };
    ...
    const handleDeleteDevice = async (id: number, name: string) => {
        ...
        try {
            ...
            alert(`Lỗi: ${error.response?.data?.error || 'Không thể xóa thiết bị'}`);
        }
    };
            ...
            <div className="flex flex-1 gap-6 overflow-hidden">
                ...
                <div className="w-2/3 bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
                    ...
                    <div className="p-5 border-b border-gray-100 bg-gray-50 flex justify-between items-center shrink-0">
                        <h3 className="font-bold text-gray-800 flex items-center gap-2"><Map size={18} /> Danh sách Phân khu</h3>
                        <button
                            onClick={() => { setAreaModalMode('add'); setAreaFormData({ id: 0, name: '', description: '', owner_id: '' }); setIsAreaModalOpen(true); }}
                            className="flex items-center gap-2 font-bold p-1.5 bg-brand-green text-white rounded-lg hover:bg-green-700 transition-colors cursor-pointer" title="Thêm khu vực mới"
                        >
                    ...
                    <div className="p-4 border-b border-gray-100 flex flex-col gap-3 shrink-0">
                        <div className="relative">
                            ...
                            <input
                                type="text" placeholder="Tìm kiếm phân khu..." value={areaSearch} onChange={(e) => setAreaSearch(e.target.value)}
                                className="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-brand-green transition-all"
                            />
                        </div>
                        <div className="relative">
                            ...
                            <select
                                ...
                                className="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-brand-green appearance-none cursor-pointer transition-all text-gray-700"
                            >
                                <option value="all">Tất cả chủ vườn</option>
                                <option value="unassigned">Chưa gán chủ vườn</option>
                                {users.map(u => (
                        ...
                        {filteredAreas.length === 0 ? (
                            ...
                            filteredAreas.map(area => (
                                <div
                                    ...
                                    className={cn(
                                        "p-4 rounded-xl border-2 cursor-pointer transition-all flex justify-between items-start group",
                                        selectedAreaId === area.id ? "border-brand-green bg-green-50" : "border-transparent bg-gray-50 hover:bg-gray-100"
                                    )}
                                >
                                    <div className="flex-1 pr-2">
                                        <p className={cn("font-bold text-base", selectedAreaId === area.id ? "text-brand-green" : "text-gray-800")}>
                                            {area.name}
                                        </p>

                                        {/* Hiển thị Tên Chủ Vườn */}
                                        <div className="mt-1.5 flex items-center gap-1.5">
                                            {area.owner_name ? (
                                                ...
                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-gray-200 text-gray-500">
                                                    ⚠️ Chưa gán chủ vườn
                                                </span>
                                            )}
                                        </div>

                                        <p className="text-xs text-gray-500 mt-1.5 line-clamp-1">{area.description || 'Chưa có mô tả'}</p>
                                    </div>

                                    {/* Nút Thao tác (Ẩn hiện khi hover) */}
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                                        <button
                                            onClick={(e) => { e.stopPropagation(); setAreaModalMode('edit'); setAreaFormData({ id: area.id, name: area.name, description: area.description, owner_id: area.owner_id ? area.owner_id.toString() : '' }); setIsAreaModalOpen(true); }}
                ...
                <div className="w-2/3 bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
                    {!selectedAreaId ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-gray-400">
                            ...
                            <p className="text-sm mt-1">Vui lòng chọn một khu vực ở danh sách bên trái</p>
                        </div>
                    ) : (
                        <>
